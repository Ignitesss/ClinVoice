...
3. В интерфейсе после 524 выполняется **повторная загрузка черновика** с сервера: если обработка успела завершиться, блоки «Транскрипт» и «Протокол» заполнятся без повторного нажатия.
