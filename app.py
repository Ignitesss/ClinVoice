...
from speechkit_webrtc import SpeechKitLiveAudioProcessor
...
def resolve_protocol_delay_sec() -> float:
    """Пауза перед запросом к LLM после автозагрузки транскрипта (сек). CLINVOICE_PROTOCOL_DELAY_SEC / Secrets."""
    raw = (os.environ.get("CLINVOICE_PROTOCOL_DELAY_SEC") or "").strip()
    if not raw:
        try:
            if hasattr(st, "secrets") and st.secrets and "CLINVOICE_PROTOCOL_DELAY_SEC" in st.secrets:
                raw = str(st.secrets["CLINVOICE_PROTOCOL_DELAY_SEC"]).strip()
        except Exception:
            pass
    default = 10.0
    if not raw:
...
def resolve_live_whisper_interval_sec() -> float:
    """
    Период отправки чанков в Yandex SpeechKit (секунды).
    **CLINVOICE_LIVE_WHISPER_INTERVAL_SEC** (env или Streamlit Secrets) — историческое имя.
    По умолчанию 5; диапазон [3, 15].
    """
...
def ensure_webrtc_shared_initialized() -> None:
    ...
    w.setdefault("recording_paused", False)
    w.setdefault("speechkit_pcm_committed", 0)
    w.setdefault("live_speechkit_text", "")
    w.setdefault("live_speechkit_error", None)


def get_cached_asr_transcriber(model_size: str, hub_model_id: str) -> "AudioTranscriberWithMetrics":
    """Один экземпляр Whisper на (hub, model_size) для финального прогона по записи."""
...
def build_speechkit_processor_factory(folder_id: str, interval_sec: float):
    """
    Фабрика WebRTC audio processor: PCM на сервере → чанки в Yandex SpeechKit (REST v1).
    Без обращения к session_state из worker — только замыкание на shared dict.
    """
    ...
    def _rec(chunk: bytes, fid: str) -> str:
        return recognize_lpcm16k_mono_chunk(chunk, fid)

    def _factory():
        return SpeechKitLiveAudioProcessor(
            shared,
            folder_id,
            _rec,
            interval_sec,
...
clinvoice_db.init_db(_SQLITE_PATH)

if not st.session_state.get("authenticated_user_id"):
    st.title("🏥 ClinVoice")
    st.caption("Войдите, чтобы продолжить.")
    with st.form("clinvoice_login"):
        _login_username = st.text_input("Логин")
        _login_password = st.text_input("Пароль", type="password")
        _login_submit = st.form_submit_button("Войти")
    if _login_submit:
        _uid_try = auth.verify_user(_SQLITE_PATH, _login_username, _login_password)
        if _uid_try is not None:
            st.session_state.authenticated_user_id = _uid_try
            _row_u = clinvoice_db.get_user_by_username(_SQLITE_PATH, _login_username)
            st.session_state.authenticated_username = (
                str(_row_u["username"]) if _row_u else _login_username
            ).strip()
            st.rerun()
        st.error("Неверный логин или пароль.")
    st.stop()

_AUTH_USER_ID = int(st.session_state.authenticated_user_id)

with st.sidebar:
    _un = st.session_state.get("authenticated_username") or ""
    if _un:
        st.caption(f"Пользователь: {_un}")
    if st.button("Выйти"):
...
class AudioTranscriberWithMetrics:
    ...
    def transcribe_audio(
        ...
        *,
        ...
        """Транскрибация аудиофайла. draft=True — быстрые настройки (только faster-whisper)."""
...
def bootstrap_consultation(db_path: str, user_id: int) -> None:
    ...
    if qp_cid and _valid_uuid_string(qp_cid):
        if clinvoice_db.consultation_exists(db_path, qp_cid, user_id):
            st.session_state.consultation_id = qp_cid
            snap = clinvoice_db.load_latest_snapshot(db_path, qp_cid, user_id)
            if snap:
                _apply_db_snapshot(snap)
            clinvoice_db.upsert_consultation_row(db_path, qp_cid, user_id)
            loaded = True
        else:
            st.warning(
                "Консультация с этим идентификатором не найдена или недоступна для вашего входа. Создана новая."
            )

    if not loaded:
        if "consultation_id" not in st.session_state:
            st.session_state.consultation_id = str(uuid.uuid4())
        clinvoice_db.upsert_consultation_row(db_path, st.session_state.consultation_id, user_id)
...
def persist_snapshot(db_path: str, user_id: int) -> None:
    cid = st.session_state.get("consultation_id")
    if not cid:
        return
    clinvoice_db.save_draft_snapshot(db_path, cid, user_id, _collect_snapshot_payload())
...
bootstrap_consultation(_SQLITE_PATH, _AUTH_USER_ID)


_live_interval = resolve_live_whisper_interval_sec()
_asr_transcriber = get_cached_asr_transcriber(model_size, hub_model_id)
_yandex_fid = (resolve_yandex_folder_id() or "").strip()

if not speechkit_configured():
    st.warning(
        "Для живого распознавания задайте **YANDEX_FOLDER_ID** и **YANDEX_CLOUD_API_KEY** "
        "или **YANDEX_IAM_TOKEN** (те же параметры, что для протокола YandexGPT)."
    )
else:
    st.caption(
        "В эфире: **Yandex SpeechKit**. Кнопка «Заполнить протокол…» сначала запускает **Whisper** "
        "по всей записи с подсказкой из черновика SpeechKit, подставляет итог в транскрипт и строит протокол."
    )

ensure_webrtc_shared_initialized()
st.checkbox(
    "Пауза записи (звук с микрофона не накапливается)",
    key="clinvoice_pause_recording",
    help="Во время паузы буфер не растёт, в SpeechKit не отправляются новые чанки.",
)
st.session_state.webrtc_shared["recording_paused"] = bool(
    st.session_state.get("clinvoice_pause_recording", False)
)

if speechkit_configured() and _yandex_fid:
    webrtc_streamer(
        key=CLINVOICE_WEBRTC_STREAMER_KEY,
        mode=WebRtcMode.SENDONLY,
        rtc_configuration=resolve_webrtc_rtc_configuration(),
        media_stream_constraints={"audio": True, "video": False},
        audio_processor_factory=build_speechkit_processor_factory(_yandex_fid, _live_interval),
        async_processing=True,
        sendback_audio=False,
        translations=WEBRTC_UI_RU,
    )
else:
    st.info("Укажите ключи Yandex Cloud — после этого появится запись с микрофона.")
...
if st.session_state.pop("_pending_webrtc_full_reset", False):
    ...
    if _rlk:
        with _rlk:
            ...
            st.session_state.webrtc_shared["live_speechkit_text"] = ""
            st.session_state.webrtc_shared["live_speechkit_error"] = None
            st.session_state.webrtc_shared["speechkit_pcm_committed"] = 0
    st.session_state.live_transcript_editor = ""
    if "clinvoice_pause_recording" in st.session_state:
        st.session_state.clinvoice_pause_recording = False
    st.session_state.webrtc_shared["recording_paused"] = False
...
with st.expander("Сохранение и восстановление консультации", expanded=False):
    ...
    with col_save:
        if st.button("Сохранить черновик сейчас", key="persist_manual_save"):
            try:
                persist_snapshot(_SQLITE_PATH, _AUTH_USER_ID)
    ...
    with col_new:
        if st.button("Новая консультация", key="persist_new_consultation"):
            ...
            try:
                clinvoice_db.upsert_consultation_row(_SQLITE_PATH, nid, _AUTH_USER_ID)
            except Exception as ex:
                st.error(str(ex))
...
def _live_transcript_ui_fragment():
    """Обновление черновика SpeechKit (только просмотр) и статус буфера."""
    ensure_webrtc_shared_initialized()
    ...
    if lk:
        with lk:
            live = sh.get("live_speechkit_text") or ""
            sec = len(sh.get("pcm_accum") or b"") / 32000.0
            err = sh.get("live_speechkit_error")
    _iv = resolve_live_whisper_interval_sec()
    st.subheader("Черновик речи (Yandex SpeechKit)")
    st.caption(
        f"Только просмотр, без редактирования. Накоплено **~{sec:.1f}** с аудио; запросы к SpeechKit — примерно каждые **{_iv:g}** с."
    )
    st.code((live.strip() or "—"), language=None)
    if err:
        st.error(err)
...
if st.button("Заполнить протокол по транскрипту", type="primary"):
    ...
    if lk:
        with lk:
            _pcm = bytes(sh.get("pcm_accum") or b"")
            _sk_live = (sh.get("live_speechkit_text") or "").strip()

    if len(_pcm) < 3200:
        st.error("В буфере слишком мало аудио (нужно хотя бы ~0,1 с записи).")
        st.stop()

    _wav_path = None
    try:
        fd, _wav_path = tempfile.mkstemp(suffix=".wav")
        os.close(fd)
        with open(_wav_path, "wb") as wf:
            wf.write(pcm_mono_s16le_to_wav_bytes(_pcm))
        _ip = _sk_live[:WHISPER_DYNAMIC_INITIAL_PROMPT_MAX_CHARS] if _sk_live else None
        with st.spinner("Whisper: уточнение транскрипта по записи (с подсказкой из SpeechKit)…"):
            _whisper_txt = transcribe_wav_in_chunks(
                _asr_transcriber,
                _wav_path,
                language="ru",
                draft=False,
        ...
        if _wav_path and os.path.isfile(_wav_path):
            try:
                os.remove(_wav_path)
            except OSError:
                pass
    ...
    if not _whisper_txt:
        st.error("Whisper вернул пустой текст — проверьте запись и модель.")
        st.stop()

    transcription = _whisper_txt
    st.session_state.live_transcript_editor = transcription
    ...
    trigger_browser_text_download("транскрипт.txt", transcription)
    time.sleep(resolve_protocol_delay_sec())
    ...
    try:
        ...
        st.success(
            "Готово. Транскрипт уточнён Whisper и подставлен ниже (только просмотр). "
            "Отредактировать можно **только блок протокола**. "
            "Текст протокола также сохраняется как **протокол.txt**; при необходимости скачайте кнопкой ниже."
        )
        try:
            persist_snapshot(_SQLITE_PATH, _AUTH_USER_ID)
        except Exception:
...
if st.session_state.original_transcription:
    ...
    with st.expander("Транскрипт (только просмотр)"):
        st.code(st.session_state.get("doctor_transcript_editor") or "—", language=None)
